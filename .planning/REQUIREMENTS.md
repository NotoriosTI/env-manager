...
- [ ] **OVERRIDE-01**: A variable can specify `origin:` to override the active environment's origin (e.g., load from gcp while active env is local)
- [ ] **OVERRIDE-02**: A variable can specify `dotenv_path:` to override the active environment's .env path when origin is local
- [ ] **OVERRIDE-03**: A variable can specify `environment:` to pin it to a specific named environment (uses that env's origin and dotenv_path)
